...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        """
        "*** YOUR CODE HERE ***"
        return len(state[1]) == 0  # If all corners have been visited
        util.raiseNotDefined()
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    for corner in corners:
        manhattenDistances.append(util.manhattanDistance(xy, corner))
    if len(manhattenDistances) > 0:
        return max(manhattenDistances)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if len(foodList) > 0:
        ...
        for food in foodList:
            ...
            )  # Use mazeDistance to calculate the real distance between two points
        return max(dists)  # The heuristic is the maximum distance to the remaining food
    return 0
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state: Tuple[int, int]):
        ...
        """
        x, y = state
        return self.food[x][y]  # Check if there's food at the current position
        "*** YOUR CODE HERE ***"
        util.raiseNotDefined()
