...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        if (
            newPos in newGhostPos
        ):  # If the new position is the same as a ghost's position
            score -= 500
        foodDistances = [
            manhattanDistance(newPos, foodPos) for foodPos in newFood.asList()
        ]
        if foodDistances:
            score += 1 / min(foodDistances)  # Closer food increases score more
        return score
...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState: GameState):
        ...
        for action in gameState.getLegalActions(0):  # Pacman's turn (maximizing player)
            successor = gameState.generateSuccessor(0, action)
            score = self.value(
                successor, 1, 1, alpha, beta
            )  # Move to the next agent (ghost)
            if score > bestScore:
                bestScore = score
                bestAction = action
